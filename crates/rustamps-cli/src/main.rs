...
impl From<VerifyProfile> for rustamps_pipeline::config::VerificationProfile {
...
fn run() -> Result<(), String> {
    ...
    let config = rustamps_pipeline::load_config_with_profile(cli.config.as_deref(), profile)
...
mod tests {
    ...
    fn existing_verify_cli_remains_valid_without_profile() {
        let cli = Cli::try_parse_from(["rustamps", "verify", "--run", "run", "--golden", "golden"])
            .unwrap();
