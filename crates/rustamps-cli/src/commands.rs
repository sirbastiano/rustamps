...
pub fn run_pipeline_command(
    args: RunArgs,
    mut config: rustamps_pipeline::RunConfig,
) -> Result<(), String> {
    if let Some(workers) = args.cpu_workers {
        config.runtime.cpu_workers = workers;
    }
    let context = rustamps_pipeline::PipelineContext {
    ...
    let report = rustamps_pipeline::run_pipeline(&context, &rustamps_pipeline::NativeExecutor)
...
pub fn status(dataset: PathBuf) -> Result<(), String> {
    let status = rustamps_pipeline::collect_status(dataset).map_err(|error| error.to_string())?;
...
pub fn verify(
    run: PathBuf,
    golden: PathBuf,
    config: rustamps_pipeline::RunConfig,
    through_stage: Option<u8>,
    final_products_only: bool,
) -> Result<(), String> {
    let report = rustamps_verify::verify_paths_with_scope(
        &run,
        &golden,
        &config.tolerance,
        through_stage,
...
pub fn prep(args: PrepArgs) -> Result<(), String> {
    match args.command {
        PrepCommand::Snap(args) => {
            let summary = rustamps_io::prepare_snap(
                &args.dataset,
                rustamps_io::SnapPrepOptions {
