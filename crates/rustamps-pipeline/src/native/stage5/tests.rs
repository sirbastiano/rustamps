...
use rustamps_core::stages::stage5::{Stage5Merged, Stage5Row};
use rustamps_io::{read_mat, write_mat, MatFile};
...
fn multi_patch_merge_requires_every_ownership_file_before_patch_io() {
    ...
    let expected_suffix = PathBuf::from("PATCH_2").join("patch_noover.in");
    assert!(message.contains(&expected_suffix.display().to_string()));
