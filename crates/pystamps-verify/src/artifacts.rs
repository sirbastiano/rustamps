...
use pystamps_io::{discover_dataset, DatasetError};
...
mod tests {
    ...
    fn discovers_patch_and_root_artifacts_in_stable_order() {
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("pystamps-artifacts-{stamp}"));
    ...
    fn through_stage_excludes_later_and_nonproduction_artifacts() {
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("pystamps-artifact-scope-{stamp}"));
        fs::create_dir_all(root.join("PATCH_1")).unwrap();
        for name in [
            "ps2.mat",
            "phuw2.mat",
    ...
    fn final_product_scope_excludes_grid_and_smoothing_intermediates() {
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("pystamps-final-products-{stamp}"));
        fs::create_dir_all(root.join("PATCH_1")).unwrap();
