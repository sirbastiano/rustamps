use crate::stage6::unwrap::local_cycles::cancel_local_negative_cycles_with_nflow;
use crate::stage6::unwrap::native::EdgeDatum;
use crate::stage6::unwrap::residual::cancel_negative_cycles_with_nflow;
...
pub(crate) fn find_negative_tree_cycle_compact(
    ...
    let mut basis = CompactTreeBasisImpl::new(view, tree_arc_indices)?;
...
pub(crate) fn optimize_tree_cycles_compact_with_state(
    ...
    for tree in &mut state.trees {
        ...
        connected = true;
        if pivot_tree {
            let mut basis = CompactTreeBasisImpl::default();
